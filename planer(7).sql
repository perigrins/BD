...
--
-- Host: localhost
-- Generation Time: Jan 29, 2024 at 02:20 PM
...
--
DELIMITER $$
CREATE TRIGGER `8_18_U` BEFORE INSERT ON `Grafik` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `grafik_dni_U` BEFORE INSERT ON `Grafik` FOR EACH ROW BEGIN
    DECLARE idGrafiku_new INT;
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `max_40h_U` BEFORE INSERT ON `Grafik` FOR EACH ROW BEGIN
    DECLARE total_hours DECIMAL(5,2);
...
(2, 'Julia', 'Nowak', 'Analityk Finansowy', 7, 9, 'Finanse2023!', 0),
(4, 'Jan', 'Kowalski', 'Inżynier', 7, 9, 'Pass456!', 0),
(5, 'Magdalena', 'Jankowska', 'Designer', 2, 9, 'Secret123!', 0),
(6, 'Piotr', 'Nowicki', 'Kierownik projektu', 8, 9, 'SecurePass789!', 0),
(7, 'Agnieszka', 'Dąbrowska', 'Specjalista ds. HR', 1, 9, 'HRpass2022@', 0),
(8, 'Tomasz', 'Kaczmarek', 'Analityk biznesowy', 5, 9, 'AnalyzePass123*', 0),
(9, 'Monika', 'Wojciechowska', 'Administrator', 6, 9, 'AdminPass789!!', 0),
(10, 'Kamil', 'Zieliński', 'Tester oprogramowania', 3, 9, 'T$est123Pass', 0),
(11, 'Karolina', 'Sikora', 'Architekt systemowy', 9, 9, 'ArchitectPass2022$$', 0),
(12, 'Marek', 'Kowalczyk', 'Ekspert ds. finansowych', 1, 9, 'FinancePass456#', 0),
(13, 'Joanna', 'Grabowska', 'Copywriter', 7, 9, 'WritePass123!', 0),
(14, 'Marcin', 'Łukasik', 'Grafik komputerowy', 4, 9, 'GraphicPass7891#', 0),
(15, 'Natalia', 'Mazur', 'Projektant UX/UI', 8, 9, 'UXPass2022!!', 0),
(16, 'Damian', 'Szymański', 'Inżynier dźwięku', 2, 9, '@udioPass123', 0),
(17, 'Iwona', 'Pawlak', 'Specjalista ds. marketingu', 6, 9, 'MarketingPass456%', 0);
...
--
DELIMITER $$
CREATE TRIGGER `15_U` BEFORE INSERT ON `Pracownicy_has_Spotkania` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `2_spr_U` BEFORE INSERT ON `Pracownicy_has_Spotkania` FOR EACH ROW BEGIN
    DECLARE uczestnicy_count INT;
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `grafik_U` BEFORE INSERT ON `Pracownicy_has_Spotkania` FOR EACH ROW BEGIN
    DECLARE work_start TIME;
    DECLARE work_end TIME;
    DECLARE meet_t TIME;
    DECLARE workday varchar(45);
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `meeting_same_hour_U` BEFORE INSERT ON `Pracownicy_has_Spotkania` FOR EACH ROW BEGIN
    DECLARE worker_count INT;
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `godzina_spotkania_U` BEFORE INSERT ON `Spotkania` FOR EACH ROW BEGIN
    DECLARE godzina_spotkania TIME;
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `maxppl_capacity_U` BEFORE INSERT ON `Spotkania` FOR EACH ROW BEGIN
    DECLARE room_capacity INT;
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `sala_spotkanie_U` BEFORE INSERT ON `Spotkania` FOR EACH ROW BEGIN
    DECLARE existing_meeting_id INT;
...
ALTER TABLE `Pracownicy_has_Spotkania`
  ...
  ADD KEY `FK_Pracownicy_has_S` (`idSpotkania`);
...
--
ALTER TABLE `Pracownicy_has_Spotkania`
  ADD CONSTRAINT `FK_Pracownicy_has` FOREIGN KEY (`idPracownika`) REFERENCES `Pracownicy` (`idPracownika`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `FK_Pracownicy_has_S` FOREIGN KEY (`idSpotkania`) REFERENCES `Spotkania` (`idSpotkania`) ON DELETE CASCADE ON UPDATE CASCADE,
